...
import com.github.TesraSupernet.exception.ExplorerException;
...
import com.github.TesraSupernet.model.dto.*;
import com.github.TesraSupernet.service.IContractService;
...
public class ContractServiceImpl implements IContractService {
    ...
    public ResponseBean queryDappBindedInfo(String dappNameArrayStr, long startTime, long endTime) {
        ...
        for (String key :
            ...
            for (Object obj :
                ...
                Map bindedTstidAndAccount = sdk.getDappBindedTstidAndAccount(paramsConfig.DAPPBIND_CONTRACTHASH, contractHash);

                Map map = (Map) valueObject.get(contractHash);
                map.putAll(bindedNodeInfo);
                map.putAll(bindedTstidAndAccount);
            }
        }
    ...
    public ResponseBean queryBindedWalletDappInfo(long startTime, long endTime) {
        ...
        for (String dappName :
            ...
            String totalFeeStr = (totalFee == null ? ConstantParam.ZERO.stripTrailingZeros().toPlainString() :
                    totalFee.multiply(ConstantParam.TSG_TOTAL).stripTrailingZeros().toPlainString());
            //激励手续费
            String rewardFeeStr = (totalFee == null ? ConstantParam.ZERO.stripTrailingZeros().toPlainString() :
                    totalFee.multiply(ConstantParam.TSG_TOTAL).multiply(new BigDecimal(paramsConfig.DAPP_REWARD_PERCENTAGE).divide(new BigDecimal("100"))).stripTrailingZeros().toPlainString());
            ...
            contractHashArray.forEach(item -> {
                ...
                // 从合约查询绑定钱包信息
                Map bindedTstidAndAccount = sdk.getDappBindedTstidAndAccount(paramsConfig.DAPPBIND_CONTRACTHASH, contractHash);
                if (Helper.isNotEmptyAndNull(bindedTstidAndAccount.get("receive_account"))) {
                    walletArray.add(bindedTstidAndAccount.get("receive_account"));
                }
            });
    ...
    public ResponseBean queryBindedNodeDappInfo(long startTime, long endTime) {
        ...
        for (String dappName :
            ...
            //激励手续费
            String rewardFeeStr = (totalFee == null ? ConstantParam.ZERO.stripTrailingZeros().toPlainString() :
                    totalFee.multiply(ConstantParam.TSG_TOTAL).multiply(new BigDecimal(paramsConfig.NODE_REWARD_PERCENTAGE).divide(new BigDecimal("100"))).stripTrailingZeros().toPlainString());
    ...
    public ResponseBean queryDappstoreDappsInfo(Integer pageSize, Integer pageNumber) {
        ...
        if (allDappstoreDapp.isEmpty()) {
            ...
            if (dappOneWeekInfoList.size() > 0) {
                ...
                for (Map map :
                        dappOneWeekInfoList) {
                    map.put("week_tst_sum", ((BigDecimal) map.get("week_tst_sum")).stripTrailingZeros().toPlainString());
                    map.put("week_tsg_sum", ((BigDecimal) map.get("week_tsg_sum")).stripTrailingZeros().toPlainString());
                    ...
                    for (Map map2 :
                        ...
                        if (dappName.equals(map2.get("dapp_name"))) {
                            map2.put("day_tst_sum", ((BigDecimal) map2.get("day_tst_sum")).stripTrailingZeros().toPlainString());
                            map2.put("day_tsg_sum", ((BigDecimal) map2.get("day_tsg_sum")).stripTrailingZeros().toPlainString());
                            map.putAll(map2);
                        }
                    }
                }
    ...
    public ResponseBean queryDappstoreDappsSummary() {
        ...
        List<Map> allDappstoreDapp = contractMapper.selectDappstoreDapp();
        if (allDappstoreDapp.isEmpty()) {
            rsMap.put("day_tst_sum", "0");
            rsMap.put("day_tsg_sum", "0");
            ...
            if (Helper.isEmptyOrNull(contractInfo)) {
                ...
                contractInfo.put("day_tst_sum", ((BigDecimal) contractInfo.get("day_tst_sum")).stripTrailingZeros().toPlainString());
                contractInfo.put("day_tsg_sum", ((BigDecimal) contractInfo.get("day_tsg_sum")).stripTrailingZeros().toPlainString());
