...
import com.github.TesraSupernet.mapper.Oep4Mapper;
...
import com.github.TesraSupernet.model.dto.BalanceDto;
import com.github.TesraSupernet.model.dto.QueryBatchBalanceDto;
import com.github.TesraSupernet.model.dto.TransferTxDetailDto;
import com.github.TesraSupernet.model.dto.TransferTxDto;
import com.github.TesraSupernet.service.IAddressService;
import com.github.TesraSupernet.util.*;
...
public class AddressServiceImpl implements IAddressService {
    ...
    private TesraSDKService sdk;

    private synchronized void initSDK() {
        if (sdk == null) {
            sdk = TesraSDKService.getInstance(paramsConfig);
        }
    }
    ...
    public ResponseBean queryAddressBalanceByAssetName(String address, String assetName) {
        ...
        if (ConstantParam.TST.equals(assetName)) {

            initSDK();
            Map<String, Object> balanceMap = sdk.getNativeAssetBalance(address);
            //TST
            BalanceDto balanceDto4 = BalanceDto.builder()
                    .assetName(ConstantParam.TST)
                    .assetType(ConstantParam.ASSET_TYPE_NATIVE)
                    .balance(new BigDecimal((String) balanceMap.get(ConstantParam.TST)))
                    .build();
            balanceList.add(balanceDto4);

        } else if (ConstantParam.TSG.equals(assetName)) {

            initSDK();
            Map<String, Object> balanceMap = sdk.getNativeAssetBalance(address);
            //TSG
            BalanceDto balanceDto1 = BalanceDto.builder()
                    .assetName(ConstantParam.TSG)
                    .assetType(ConstantParam.ASSET_TYPE_NATIVE)
                    .balance((new BigDecimal((String) balanceMap.get(ConstantParam.TSG)).divide(ConstantParam.TSG_TOTAL)))
                    .build();
            balanceList.add(balanceDto1);

            //waiting bound TSG
            String waitBoundTsg = calculateWaitingBoundTsg(address, (String) balanceMap.get(ConstantParam.TST));
            BalanceDto balanceDto2 = BalanceDto.builder()
                    .assetName(ConstantParam.WAITBOUND_TSG)
                    .assetType(ConstantParam.ASSET_TYPE_NATIVE)
                    .balance((new BigDecimal(waitBoundTsg)))
                    .build();
            balanceList.add(balanceDto2);

            //Claimable TSG
            String unBoundTsg = sdk.getUnBoundTsg(address);
            if (Helper.isEmptyOrNull(unBoundTsg)) {
                unBoundTsg = "0";
            }
            BalanceDto balanceDto3 = BalanceDto.builder()
                    .assetName(ConstantParam.UNBOUND_TSG)
                    .assetType(ConstantParam.ASSET_TYPE_NATIVE)
                    .balance((new BigDecimal(unBoundTsg)))
                    .build();
     ...
     * @return
     */
    private String calculateWaitingBoundTsg(String address, String tst) {

        Integer latestTstTransferTxTime = null;
        //mysql 4.0.14+ bug
        try {
            latestTstTransferTxTime = txDetailMapper.selectLatestTstTransferTxTime(address);
        } catch (Exception e) {
            log.error("{} error...", Helper.currentMethod(), e);
        }

        if (Helper.isEmptyOrNull(latestTstTransferTxTime)) {
            return "0";
        }
        long now = System.currentTimeMillis() / 1000L;
        log.info("calculateWaitingBoundTsg latestTstTransferTxTime:{},now:{}", latestTstTransferTxTime, now);
        BigDecimal totalTsg = new BigDecimal("0");
        //before 20190630000000 UTC
        if (latestTstTransferTxTime < TIMESTAMP_20190630000000_UTC) {
            BigDecimal tsg01 = new BigDecimal(TIMESTAMP_20190630000000_UTC).subtract(new BigDecimal(latestTstTransferTxTime)).multiply(new BigDecimal(5));
            BigDecimal tsg02 = new BigDecimal(now).subtract(new BigDecimal(TIMESTAMP_20190630000000_UTC)).multiply(paramsConfig.TSG_SECOND_GENERATE);
            totalTsg = tsg01.add(tsg02);
        } else {
            totalTsg = new BigDecimal(now).subtract(new BigDecimal(latestTstTransferTxTime)).multiply(paramsConfig.TSG_SECOND_GENERATE);
        }
        BigDecimal tsg = totalTsg.multiply(new BigDecimal(tst)).divide(ConstantParam.TST_TOTAL);

        return tsg.toPlainString();
    }
    ...
    private List<TransferTxDto> formatTransferTxDtos(List<TransferTxDto> transferTxDtos) {
        ...
        for (int i = 0; i < transferTxDtos.size(); i++) {
            ...
            if (ConstantParam.TSG.equals(assetName)) {
                amount = amount.divide(ConstantParam.TSG_TOTAL);
            }
