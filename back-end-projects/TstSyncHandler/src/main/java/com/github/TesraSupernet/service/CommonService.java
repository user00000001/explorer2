...
public class CommonService {
    ...
    /**
     * switch to another node and initialize TST_SDKSERVICE object
    ...
    public void switchNode() {
        ...
        ConstantParam.TST_SDKSERVICE = wm;
    }

    /**
     * get the the blockheight of the tesra blockchain
     *
     * @return
     * @throws Exception
     */
    public int getRemoteBlockHeight() throws Exception {
        ...
        while (true) {
            try {
                remoteHeight = ConstantParam.TST_SDKSERVICE.getConnect().getBlockHeight();
                break;
    ...
    public JSONObject getBlockJsonByHeight(int height) throws Exception {
        ...
        while (true) {
            try {
                blockObj = (JSONObject) ConstantParam.TST_SDKSERVICE.getConnect().getBlockJson(height);
                break;
            } catch (ConnectorException ex) {
    ...
    public JSONArray getTxEventLogsByHeight(int height) throws Exception {
        ...
        while (true) {
            try {
                ...
                Object object = ConstantParam.TST_SDKSERVICE.getConnect().getSmartCodeEvent(height);
    ...
    public Long getOep5TotalSupply(String contractAddress) {
        ...
        ConstantParam.TST_SDKSERVICE.neovm().oep5().setContractAddress(contractAddress);
        Long totalSupply = 0L;
        int tryTime = 1;
        while (true) {
            try {
                totalSupply = Long.valueOf(ConstantParam.TST_SDKSERVICE.neovm().oep5().queryTotalSupply());
                break;
            } catch (ConnectorException ex) {
    ...
    public JSONObject getEventLogByTxHash(String txHash) throws Exception {
        ...
        while (true) {
            try {
                eventObj = (JSONObject) ConstantParam.TST_SDKSERVICE.getConnect().getSmartCodeEvent(txHash);
                break;
            } catch (ConnectorException ex) {
    ...
    public JSONObject getContractInfoByTxHash(String txHash) throws Exception {
        ...
        while (true) {
            try {
                DeployCode deployCodeObj = (DeployCode) ConstantParam.TST_SDKSERVICE.getConnect().getTransaction(txHash);
                //根据code转成合约hash
                ...
                try {
                    contractObj = (JSONObject) ConstantParam.TST_SDKSERVICE.getConnect().getContractJson(contractHash);
    ...
    public Long getOep8TotalSupply(String tokenId) {
        ...
        while (true) {
            try {
                totalSupply = Long.valueOf(ConstantParam.TST_SDKSERVICE.neovm().oep8().queryTotalSupply(Helper.hexToBytes(tokenId)));
                break;
            } catch (ConnectorException ex) {
